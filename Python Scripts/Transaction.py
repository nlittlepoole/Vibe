...
while count[0][0] != 0:
    ...
    for affiliate in affiliations:
        ...
        if len(data[0])>1 and len(data[1])>1:
            ...
            if exist[0][0]>0:
                ...
                cur.execute(query)
                cur.connection.commit()
                query="CREATE TABLE IF NOT EXISTS `"+ data[1]+"` (  `ID` int(10) NOT NULL AUTO_INCREMENT,  `Attribute` varchar(255) NOT NULL,  `Keywords` varchar(255) NOT NULL,  `Average` float NOT NULL DEFAULT '0',  `Sum` int(10) NOT NULL DEFAULT '0',  `Squares` int(10) NOT NULL DEFAULT '0',  `Deviation` float NOT NULL DEFAULT '0',  `Rank1` varchar(255) NOT NULL,  `Rank2` varchar(255) NOT NULL,  `Rank3` varchar(255) NOT NULL,  `Rank4` varchar(255) NOT NULL,  `Rank5` varchar(255) NOT NULL,  `Rank6` varchar(255) NOT NULL,  `Rank7` varchar(255) NOT NULL,  `Rank8` varchar(255) NOT NULL,  `Rank9` varchar(255) NOT NULL,  `Rank10` varchar(255) NOT NULL,  PRIMARY KEY (`ID`)) ENGINE=InnoDB  DEFAULT CHARSET=latin1 AUTO_INCREMENT=15 ;"
                cur.execute(query)
                cur.connection.commit()
    ...
    if exists:
        ...
        #print score
        if newKeyword:
            query="UPDATE user SET " + attribute + "=" +""+ score+ " , " + attribute + "_Total="+attribute+"_Total + 1-"+str(modifier)+" ," +attribute + "_Keywords='" +newKeyword + "',Comments='"+comment+"', Gender='"+gender+"', newAnswers=newAnswers+1,totalAnswers=totalAnswers+1 WHERE UID="+ user2
        #print query
        else:
            query="UPDATE user SET " + attribute + "=" +""+ score+ " , " + attribute + "_Total="+attribute+"_Total + 1-"+str(modifier)+",Comments='"+comment+"', Gender='"+gender+"' , newAnswers=newAnswers+1,totalAnswers=totalAnswers+1  WHERE UID="+ user2
        cur.execute(query)
        cur.connection.commit()
    else:
        if not "null" in keyword:
            keyword="1"+keyword
        query="INSERT INTO user (UID, ACTIVE, " + attribute + "," + attribute + "_Total ," + attribute+ "_Keywords,Comments,Communities,Gender,Name,newAnswers,totalAnswers) VALUES (" + user2 + ",0,"+ str(score)+","+str(1-modifier)+", '"+keyword+"','"+comment+"','"+affiliation+"','"+gender+"','"+name+"',1,1)"
        print query
        cur.execute(query)
        cur.connection.commit() 
